...
const TourList = ({ tours: propTours, paginated = true }) => {
  const { tours: contextTours } = useContext(TourContext);
  const navigate = useNavigate();
  const [currentPage, setCurrentPage] = useState(1);

  const tours = propTours || contextTours;
  const totalTours = tours?.length || 0;
  const totalPages = Math.ceil(totalTours / TOURS_PER_PAGE);

  const displayTours = paginated
    ? tours?.slice(
    ...
    <section className="py-12 bg-white">
      <div className="max-w-7xl mx-auto px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {(displayTours?.length ? displayTours : [...Array(6)]).map(
            (tour, index) => (
              <div
                key={index}
              ...
              >
                ...
                <div className="p-4 flex flex-col gap-2 flex-1">
                  ...
                  <div className="flex items-center gap-1 text-gray-500 text-xs text-left">
                    ...
                    <span>{tour?.maxGroupSize || "Vịnh Hạ Long"}</span>
                  </div>
                  ...
                  {/* Description */}
                  <div className="flex items-center gap-2 text-gray-600 text-sm text-left min-h-[48px]">
                    <svg
                      className="h-5 w-5"
                      fill="none"
                      stroke="currentColor"
        ...
        {paginated && totalPages > 1 && (
          <div className="flex justify-center mt-8 gap-2">
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                key={i}
                className={`px-3 py-1 rounded border ${currentPage === i + 1 ? "bg-cyan-400 text-white" : ""}`}
